...
export class Model<T> {
  ...
  get<Key extends keyof T>(key: string, defaultValue?: unknown): unknown {
    return dotProp.get<T[Key] | undefined>(this.data, key, defaultValue as T[Key]);
  }
  ...
  set<Key extends keyof T>(key: string, value: T[Key] | ((draft: T[Key]) => void)): void {
    ...
    if (typeof value === 'function') {
      const cb = value as (draft: T[Key]) => void;
      this.data = produce(this.data, (draft) => {
        cb(dotProp.get(draft, key));
      });
    } else {
      checkValueType(key, value);
      this.data = produce(this.data, (draft) => {
        dotProp.set(draft, key, value);
      });
    }
  }
